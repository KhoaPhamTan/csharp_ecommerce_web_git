        ...
        public void OnGet(string categoryName)
        {
            ...
            if (petStoresQuery != null)
            {
                PetStores = petStoresQuery.Include(p => p.Category)
                    ...
                    .Select(p => new PetStoreDTO(p.Id, p.PetName, p.Category.Name, p.Gender, p.PetDescription, p.Price, DateOnly.FromDateTime(p.BirthDay), p.ImageUrl.StartsWith("/images/") ? p.ImageUrl : "/images/" + p.ImageUrl)) // Ensure ImageUrl is correctly set
                    .ToList();
            }
